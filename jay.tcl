...
########################################
## Jay error and notification dialogs ##
########################################
...
# Where:
#
# message       Should be a list that specifies the message to display.
#               The message will be auto translated, if possible.
#               In order to do so, two things are required:
#                     1 - The text string should be provided in english ('en').
#
...
#                         Example1: No substitution string
#
#                                 ::_NOTIFY [list "Settings:"]    or just     ::_NOTIFY "Settings:"
#
#                         Example2: One substitution string
#
#                                 ::_NOTIFY [list "The %s command is invalid." grid]
#
#                         Example3: Two substitution strings
#
#                                 ::_NOTIFY [list "The %s command address is invalid: '%s'" grid .myapp]
#
#                         ...
#
#                     2 - A message catalog should be provided with all the translations needed by the application.
...
proc ::_NOTIFY { message } {
    ...
    switch -- $::TEMP(init,state) {
            ...
            #        As a fallback the message provided will be displayed in the standard output channel.
            #        No translation or substitutions will be done atm.
            puts stdout "Message: [lindex $message 0]"
        }
        ongoing {
            # Note:  Jay is not fully initialized yet, we will fallback to use the Tk widgets.
            ...
            wm attributes .notify {*}$ATTRIBUTES
            wm minsize    .notify 324 200
            wm protocol   .notify WM_DELETE_WINDOW [list set ::TEMP(response) OK]
            wm title      .notify [::msgcat::mc "Notification"]
            ...
            ::ttk::label .notify.f.info_img       -anchor w \
                                                   -class TLabel \
                                                -compound image \
                                                  -cursor "" \
                                                    -font "" \
            ...
            ::ttk::checkbutton .notify.f.cb        -class TCheckbutton \
                                                -compound none \
                                                  -cursor "" \
                                                   -image [list ] \
                                                   -state normal \
                                                   -style TCheckbutton \
                                               -takefocus 1 \
                                                    -text [::msgcat::mc "Don't show these kind of messages anymore."] \
                                            -textvariable "" \
                                               -underline -1 \
                                                   -width 0 \
                                                 -command "" \
            ...
            set BUTTON_OPTIONS  [list              -class TButton \
                                                 -command [list set ::TEMP(response) OK] \
                                                -compound text \
                                                  -cursor "" \
                                                 -default active \
                                                   -image [list ] \
            ...
            grid .notify.f.info_img     -column 0 \
                                          -padx [list 2p 7p] \
                                          -pady [list 13p 7p] \
                                           -row 0 \
                                        -sticky nsew;

            grid .notify.f.msg          -column 1 \
                                          -padx [list 7p 2p] \
                                          -pady [list 13p 7p] \
                                           -row 0 \
                                        -sticky ew;

            grid .notify.f.cb           -column 1 \
                                          -padx [list 7p 2p] \
                                          -pady [list 13p 7p] \
                                           -row 1 \
                                        -sticky ew;
            ...
            # Bindings.
            bind .notify.btn    <KeyPress-Return>   [list set ::TEMP(response) OK]
            bind .notify.btn    <KP_Enter>          [list set ::TEMP(response) OK]
            bind .notify        <KeyPress-Escape>   [list set ::TEMP(response) OK]
            bind .notify        <Map>               [list ::_CENTER_ON_THE_SCREEN .notify]
